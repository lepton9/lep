...
static void synchronize_statement(parser *p) {
  ...
  if (p->token.type == T_SEMICOLON) get_next(p);
}
...
token *get_next(parser *p) {
  p->token = p->lookahead;
  if (p->lookahead.type != T_EOF) p->lookahead = next_token(p->lexer);
  return &p->token;
}

static inline const token* peek_token(const parser* p) {
  return &p->lookahead;
}

AST* parse_program(parser* p) {
  AST* program = init_ast(AST_PROGRAM, NULL);
  AST* cur = program;
...
AST* parse_id(parser* p) {
  ...
  AST* ast = init_ast(AST_ID, &t);
  return ast;
}

AST* parse_type(parser* p) {
  AST* type = init_ast(AST_TYPE, &p->token);
  accept_type(p);
  return type;
}
...
AST* parse_func_def(parser* p) {
  ...
  AST* func = init_ast(AST_FUNCTION, &t);
  func->l = header;
  func->r = body;
  return func;
}
...
AST* parse_func_header(parser* p) {
  ...
  if (p->token.type == T_KW_MAIN) {
    name = init_ast(AST_MAIN, &p->token);
    expect(p, T_KW_MAIN);
  } else {
    name = parse_id(p);
  }
  ...
  AST* header = init_ast(AST_FHEADER, NULL);
  header->l = name;
  name->next = params;
...
AST* parse_func_params(parser* p) {
  AST* params = init_ast(AST_PARAMETERS, NULL);
...
AST* parse_func_param(parser* p) {
  return (is_type(p)) ? parse_var_decl(p) : NULL;
}
...
AST* parse_primary(parser *p) {
  ...
  token t = p->token;
  if (accept(p, T_BANG)) {
    term = init_ast(AST_OPERATOR, &t);
    term->l = parse_primary(p);
  } else if (p->token.type == T_IDENTIFIER && peek_token(p)->type == T_PAREN_L) {
    term = parse_fcall(p);
  } else if (accept(p, T_IDENTIFIER)) {
    term = init_ast(AST_ID, &t);
  } else if (accept_literal(p)) {
    term = init_ast(AST_VALUE, &t);
...
AST* parse_var_decl(parser* p) {
  ...
  accept_type(p);
  expect(p, T_COLON);
  AST* id = init_ast(AST_ID, &p->token);
  expect(p, T_IDENTIFIER);
  AST* var = init_ast(AST_VARIABLE, NULL);
  var->l = type;
  var->r = id;
  if (p->token.type == T_EQUALS) {
    AST* assignment = init_ast(AST_ASSIGNMENT, &p->token);
    expect(p, T_EQUALS);
...
AST* parse_fcall_args(parser* p) {
  AST* args = init_ast(AST_FARGUMENTS, NULL);
...
AST* parse_fcall(parser* p) {
  ...
  AST* func_call = init_ast(AST_FCALL, NULL);
  func_call->l = id;
  func_call->r = args;
  expect(p, T_PAREN_R);
  return func_call;
...
AST* parse_block(parser* p) {
  expect(p, T_BRACE_L);
  AST* block = init_ast(AST_BLOCK, NULL);
...
AST* parse_return(parser *p) {
  ...
  AST* ret_node = init_ast(AST_RET, &t);
...
AST* parse_while(parser *p) {
  ...
  expect(p, T_KW_WHILE);
  expect(p, T_PAREN_L);
  AST *while_node = init_ast(AST_WHILE, &t);
...
AST* parse_multiplicative_expr(parser* p) {
  ...
  while (p->token.type == T_ASTERISK || p->token.type == T_SLASH) {
    token op = p->token;
    get_next(p);
    AST* op_node = init_ast(AST_OPERATOR, &op);
    op_node->l = expr;
